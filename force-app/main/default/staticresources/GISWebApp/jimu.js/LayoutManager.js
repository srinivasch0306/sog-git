...
function(declare, lang, array, html, _WidgetBase, topic, on, query, domConstruct, domGeometry,
  ...
  MapManager, utils, OnScreenWidgetIcon, WidgetPlaceholder, LoadingShelter, tokenUtils, Message) {
  ...
  clazz = declare([_WidgetBase], {
    ...
    _loadMap: function() {
      ...
      if(this.appConfig.tokenInfo && this.appConfig.tokenInfo.tokenServicesUrl && this.appConfig.tokenInfo.tokenServicesUrl != '' && this.appConfig.tokenInfo.serviceUrl && this.appConfig.tokenInfo.serviceUrl != '' && !this.appConfig.tokenInfo.isUserLogin){
	...
	_getSecurityToken: function(tokenObj){
	 var bgError = this._isBGError(tokenObj);
	 if(bgError == true)
	 {
		return;
	 }
	...
	_isBGError: function(result) {
        if(typeof result === 'string' && result.indexOf('Exception:') != -1) {
          new Message({
            message: result
          });
          //this.shelter.hide();
    ...
    _loadPreloadWidgets: function(appConfig) {
      ...
      all(defs).then(lang.hitch(this, function(){
		...
		if(zoomOnLoad == 'true'){
			this.preloadWidgetIcons[5].onClick(); // 5 = eSearch widget
		}
		//BG try to open Search widget here
		else if(showDirections == 'true'){
			this.preloadWidgetIcons[4].onClick(); // 4 = Directions widget
		}
        this._doPostLoad();
