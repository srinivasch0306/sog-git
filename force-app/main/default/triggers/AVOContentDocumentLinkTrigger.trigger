trigger AVOContentDocumentLinkTrigger on ContentDocumentLink (after insert) {
            ...
            if (permit != null && 
                (permit.MUSW__Status__c == 'Permit Issued' || permit.MUSW__Status__c == 'Permit Denied' || 
                 ...
                 permit.MUSW__Status__c == 'Cancelled' || permit.MUSW__Status__c == 'Consolidated')) {
                     ...
                     // AvoSharePointUtil.transferBegin(cdl);
    ...
    /* Boolean isOnlyMeterUpload = false;
...
(
doc.LinkedEntityId.getSObjectType() == AVO_Meter__c.SObjectType ||
doc.LinkedEntityId.getSObjectType() == AVO_Meter_Codeset__c.SObjectType ||
doc.LinkedEntityId.getSObjectType() == AVO_Meter_Transaction__c.SObjectType ||
doc.LinkedEntityId.getSObjectType() == AVO_Meter_Withdrawal__c.SObjectType
)
) 
{
isOnlyMeterUpload = true;
break;
}
}
if (isOnlyMeterUpload) {
//Automatic Meter Move Files
for (ContentDocumentLink doc : Trigger.new) {
System.debug('Triggering SharePoint transfer for AVO_Meter__c ID: ');
// if (doc != null && doc.LinkedEntityId != null && doc.LinkedEntityId.getSObjectType() == AVO_Meter__c.SObjectType) 
if (
doc != null && 
doc.LinkedEntityId != null && 
(
...
// System.enqueueJob(new AVO_SharePointQueueable(doc.ContentDocumentId, doc.LinkedEntityId));
AVO_SharePointProcessRecord.transferBegin(doc);
System.debug('Finished SharePoint transfer enqueue for document ID: ' + doc.ContentDocumentId);
}
System.debug('END Triggering SharePoint transfer for AVO_Meter__c ID: ');
}
}
else
{
